...
class VAERunner():
    ...
    def __init__(self, config: Dict) -> None:
        ...
        if self.optimise_local:
            self.localised_optimisation_network = self._setup_local_optimisation(config)
    ...
    def _extract_parameters(self, config: Dict) -> None:
        ...
        if self.optimise_local:
            self.manual_saved_model_path = config.get(["local_optimisation", "manual_saved_model_path"])

            self.use_balanced_dataset = config.get(["local_optimisation", "use_balanced_dataset"])

            self.max_num_epochs = config.get(["local_optimisation", "max_num_epochs"])
            self.convergence_check_period = config.get(["local_optimisation", "convergence_check_period"])
            self.cycles_until_convergence = config.get(["local_optimisation", "cycles_until_convergence"])
            self.local_mc_samples = config.get(["local_optimisation", "mc_samples"])
            self.local_approximate_posterior = config.get(["local_optimisation", "approximate_posterior"])
            self.local_num_batches = config.get(["local_optimisation", "num_batches"])
            ...
            # self.factor = config.get(["local_optimisation", "encoder", "output_dimension_factor"]) // 2
    ...
    def _setup_dataset(self, config: Dict):
        ...
        if self.dataset == "mnist":
            ...
            balanced = self.optimise_local and self.use_balanced_dataset
            dataloader = cifar_dataloader(data_path=os.path.join(file_path, self.relative_data_path), batch_size=self.batch_size, train=True, balanced=balanced)
    ...
    def _setup_local_optimisation(self, config: Dict):

        if self.local_approximate_posterior == "gaussian":
            local_optimisation_module = GaussianLocalOptimisation(config=config)
        elif self.local_approximate_posterior == "rnvp_norm_flow":
            local_optimisation_module = RNVPLocalOptimisation(config=config)
        elif self.local_approximate_posterior == "rnvp_aux_flow":
            local_optimisation_module = RNVPAuxLocalOptimisation(config=config)
        elif self.local_approximate_posterior == "planar_flow":
            local_optimisation_module = PlanarLocalOptimisation(config=config)
        else:
            raise ValueError("Approximate posterior family {} not recognised for local optimisation".format(local_approximate_posterior))

        return local_optimisation_module
    ...
    def _load_checkpointed_model(self, model_path: str) -> None:
        ...
        if not os.path.exists(model_path):
            raise FileNotFoundError("Saved weights for specified config could not be found in specified path. \
                                    To train locally optimised inference parameters, pretrained model is required.")
        else:
    ...
    def train_local_optimisation(self) -> None:
        """
        Local optimisation (per data batch) of latent distribution parameters.
        ...
        for b, (batch_input, _) in enumerate(self.dataloader, start=1): # target discarded
            ...
            print("Locally optimising latent distribution parameters for data batch {}/{}".format(b, self.local_num_batches))
            ...
            additional_optimisation_parameters = self.localised_optimisation_network.get_additional_parameters()

            parameters_to_optimise = [{'params': [mean, logvar]}, {'params': additional_optimisation_parameters}]

            local_optimiser = self.localised_optimisation_network.get_local_optimiser(parameters=parameters_to_optimise)
            ...
            for e in range(1, self.max_num_epochs + 1):
                ...
                z, params = self.localised_optimisation_network.sample_latent_vector([mean, logvar])
            ...
            # evaluation
            test_z, test_params = self.localised_optimisation_network.sample_latent_vector([mean, logvar])
